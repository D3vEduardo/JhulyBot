...
new Event({
    ...
    async run(interaction) {
        ...
        users.forEach(async user => {
            ...
            if ( user.id === interaction.user.id || !userData.userDetails.status ) return;
            const userJobStatus = userData.userDetails.status.find(i => i.name === "job");
            if ( userJobStatus && userJobStatus!.expire > Date.now() ) return; // ? Caso o user não tenha terminado de trabalhar
            
            // ? Caso o user tenha terminado de trabalhar
            const userJob = jobs[userJobStatus!.data as keyof typeof jobs];
