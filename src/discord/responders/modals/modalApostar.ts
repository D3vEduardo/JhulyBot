...
import { ButtonBuilder, ButtonStyle, codeBlock, ModalMessageModalSubmitInteraction } from "discord.js";
...
async function Dados(interaction: ModalMessageModalSubmitInteraction<"cached">) {
    ...
    await interaction.reply({ embeds: [Embed(interaction).setDescription("Sorteando o seu dado...")] });

    await delay(Math.floor(Math.random()*5000)+1);

    await interaction.editReply({ embeds: [Embed(interaction).setDescription("Sorteando o dado da máquina...")] });
...
async function PedraPapelTesoura(i: ModalMessageModalSubmitInteraction<"cached">) {
    const embed = Embed(i).setDescription("Você quer :mountain:`pedra`, :scroll:`papel` ou :scissors:`tesoura`?");
    const row = createRow(
        new ButtonBuilder({
            customId: "PedraPapelTesoura/pedra",
            emoji: ":mountain:",
            label: "Pedra",
            style: ButtonStyle.Secondary
        }),
        new ButtonBuilder({
            customId: "PedraPapelTesoura/papel",
            emoji: ":scroll:",
            label: "Papel",
            style: ButtonStyle.Primary
        }),
        new ButtonBuilder({
            customId: "PedraPapelTesoura/tesoura",
            emoji: ":scissors:",
            label: "Tesoura",
