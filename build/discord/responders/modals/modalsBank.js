...
import { FirebaseGet, FirebaseSet } from "#database";
...
new Responder({
    ...
    async run(interaction) {
        ...
        const AuthorData = await FirebaseGet(Author.id);
        let UserRecivePix;
        try {
            ...
            await interaction.update({
                embeds: [
                    Embed(interaction)
                        ...
                        .setDescription(`${settings.emojis.error} ${Author}, chave pix inválida!`),
                ],
                components: [],
            });
            return;
        ...
        if (isNaN(valueNumberPix) || valueNumberPix <= 0) {
            await interaction.update({
                embeds: [
                    Embed(interaction)
                        .setColor(settings.colors.danger)
                        .setDescription(`${settings.emojis.error} ${Author}, o **valor** da __transferência__ pix não é um __número__ **válido**!`),
                ],
                components: [],
            });
            return;
            ...
            await interaction.update({
                embeds: [
                    Embed(interaction)
                        .setColor(settings.colors.danger)
                        .setDescription(`${settings.emojis.error} ${Author}, você **não** possuí \`${FormatSaldo(valueNumberPix)}\` em sua conta __JhulyBank__!`),
                ],
                components: [],
            });
            return;
        ...
        const row = createRow(new ButtonBuilder()
            .setCustomId(`ButtonSendPix/${valueNumberPix}/${(await UserRecivePix).id}/${Author.id}`)
            .setLabel("Enviar pix")
        ...
        await interaction.reply({
            ...
            embeds: [
                Embed(interaction)
                    ...
                    .setDescription(`${settings.emojis.warning} ${Author} você **realmente** deseja enviar um __pix__ no valor de \`${FormatSaldo(valueNumberPix)}\` para \`${(await UserRecivePix).displayName} (${(await UserRecivePix).id})\`?`),
            ],
        });
    },
});
    ...
    async run(interaction) {
        ...
        if (isNaN(valueNumberDep) || valueNumberDep <= 0) {
            await interaction.update({
                components: [],
                embeds: [
                    Embed(interaction).setDescription(`${settings.emojis.error} ${Author}, \`${valueStringDep}\` não é um número válido!`),
                ],
            });
            return;
        }
            ...
            await interaction.update({
                ...
                embeds: [
                    Embed(interaction)
                        .setDescription(`${settings.emojis.error} ${Author}, você não possuí \`${FormatSaldo(valueNumberDep)}\` em sua conta JhulyBank.`)
                        .setColor(settings.colors.danger),
                ],
            });
            return;
        ...
        await FirebaseSet(Author.id, AuthorData);
        await interaction.update({
            components: [],
            embeds: [
                Embed(interaction)
                    .setColor(settings.colors.success)
                    .setDescription(`${settings.emojis.success} O __depósito__ foi realizado com **sucesso**!\nEnviamos mais __detalhes__ na **DM**.`),
            ],
        });
        const now = new Date();
        ...
        const DmEmbed = Embed(interaction)
    ...
    async run(interaction) {
        ...
        if (isNaN(numberValueSaque) || numberValueSaque <= 0) {
            await interaction.update({
                ...
                embeds: [
                    Embed(interaction)
                        .setDescription(`${settings.emojis.error} ${Author}, \`${stringValueSaque}\` não é um número válido!`)
                        .setColor(settings.colors.danger),
                ],
            });
            return;
            ...
            await interaction.update({
                ...
                embeds: [
                    Embed(interaction)
                        .setDescription(`${settings.emojis.error} ${Author}, você não possuí \`${FormatSaldo(numberValueSaque)}\` em sua conta JhulyBank.`)
                        .setColor(settings.colors.danger),
                ],
            });
            return;
        ...
        await interaction.update({
            ...
            embeds: [
                Embed(interaction)
                    .setColor(settings.colors.success)
                    .setDescription(`${settings.emojis.success} ${Author}, o __saque__ foi **concluído** com sucesso!\nTe enviamos mais __detalhes__ na **DM**.`),
            ],
        });
        const now = new Date();
